...
for file in os.listdir(path):
    ...
    if (file.count("-0_at") == 1) and (file[:2] == "90"):
        vars()["I_0_90_"+file[3:a]] = np.loadtxt(path+file, unpack=False, skiprows=6)[:,1]*1.5
    elif (file.count("-90_at") == 1) and (file[:2] == "90"):
        vars()["I_90_90_"+file[3:a]] = np.loadtxt(path+file, unpack=False, skiprows=6)[:,1]*1.5
    elif (file.count("-unpol_at") == 1) and (file[:2] == "90"):
        vars()["I_UN_90_"+file[3:a]] = np.loadtxt(path+file, unpack=False, skiprows=6)[:,1]*1.5
